...
function Scenery({ desktop, mobile, className, alt = "" }: { desktop: string; mobile: string; className: string; alt?: string }) {
  return <picture><source media="(max-width: 700px)" srcSet={art + mobile} /><img className={className} src={art + desktop} alt={alt} decoding="async" /></picture>;
}
export default function Hero() {
  ...
  return <section id="home" className="hero" ref={ref} aria-label="Wedding invitation">
    <div className="hero__stage">
      ...
      <h1 className="hero__names"><span className="scene-name">{wedding.groom.name}</span><span className="scene-weds">weds</span><span className="scene-name">{wedding.bride.name}</span></h1>
      <div className="hero__invite">
        <div id="invitation" className="invite-ganesh-heading" lang="hi">॥ श्री गणेशाय नमः ॥</div>
        <LivingChakra />
        ...
        <p className="invite-request">With the blessings of Lord Ganesha, we joyfully invite you to the wedding of our beloved son</p>
        <p className="invite-name">{wedding.groom.name}</p><p className="invite-and">with</p><p className="invite-name">{wedding.bride.name}</p>
        <p className="invite-relation">Beloved daughter of</p><p className="invite-family">{wedding.bride.daughterOf}</p>
        ...
        <p className="invite-story">Walk with us from the first joyful rituals<br />to the beginning of their forever.</p>
        <a className="scene-arrow" href="#events" aria-label="Discover our wedding celebrations">↓</a>
      </div>
      <div className="hero-petals" aria-hidden="true"><Petals count={8} /></div>
